...
def run(rank, n_gpus, hps):
  ...
  try:
    _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.model_dir, "G_mb.pth"), net_g, optim_g)
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
    ...
    if rank==0:
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        utils.save_checkpoint(net_g, optim_g, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "G_mb.pth".format(global_step)))
        utils.save_checkpoint(net_d, optim_d, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "D_mb.pth".format(global_step)))
    global_step += 1
